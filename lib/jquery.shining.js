  ...
  $.shining = function() {
    ...
    $.fn.hasClasses = function(classes) {
      var classes = classes.split(/\s+/),
        yes = false,
        element = this.get(0);
      for (var i = 0; i < classes.length; i++) { yes = $(element).hasClass(classes[i]) }
      return yes;
    }
    ...
    var COUNTER = {
      ...
      'fades-out': ['fades-in', 'transparent']
    }
    ...
    function applyEffect(element, name) {
      if (name in COUNTER && $(element).hasClasses(COUNTER[name][1])) {
        $(element).removeClass(COUNTER[name][0])
      } else {
    ...
    function getSlide(name) {
      ...
      if ($.shining.config.transitions) {
        $('#stage').effect('fades-out');
        setTimeout(
          function() {
            loadSlide(name);
            setTimeout(
              function() {
                $('#stage').effect('fades-in');
    ...
    function loadConfig(callback) {
      $.getJSON('config.json', function(config) {
        ...
        if (config.transitions) { $('#stage').effect('transparent'); }
        callback.call();
      });
    }
