...
(function($) {
  ...
  function setTheme(name) {
    ...
    if ($('link.theme').length) {
      return $('link.theme').attr('href', "vendor/themes/" + name + ".css");
    } else {
  ...
  when('slideloaded', function(event, name) {
    console.log('loading: ' + name + ' - loaded ' + Shining.slides.totalLoaded() + ' out of ' + Shining.slides.length())
  })
  ...
  $(document)
    ...
    .ready(function() {
      loadConfig(function() {
        ...
        loadSlide(first, function() { playSlide(first); centerStage(); preloadSlides() });
