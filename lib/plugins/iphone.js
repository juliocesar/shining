...
if (navigator.userAgent.match(/iPhone/i)) {
  // Lay all slides inline one next to another
  ...
  Shining.slides.nextSlide = function() {
    if (Shining.slides.next()) {
      var offset = Shining.slides._slides.indexOf(Shining.slides.next()) * getSlideWidth();
      $('body').transformTransition({translate: {x: -offset }});
      Shining.slides.current(Shining.slides.next());
    }
  }

  Shining.slides.previousSlide = function() {
    if (Shining.slides.previous()) {
      var offset = Shining.slides._slides.indexOf(Shining.slides.previous()) * getSlideWidth();
      $('body').transformTransition({translate: {x: -offset }});
      Shining.slides.current(Shining.slides.previous());
    }
  }

  function fitToView() {
    $('div.slide').each(function(i) {
      $(this).css({left: i * getSlideWidth() });
    });
  }
  ...
  function flick(dir) {
    return dir == "right" ? Shining.slides.nextSlide() : Shining.slides.previousSlide();
	}
