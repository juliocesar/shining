...
// The iPhone touch code is heavily borrowed from libraries around the web. If you'd like credit for anything, get in touch.
...
if (iPhone || iPad) {
  ...
  Shining.slides.playSlide = function(name) {
    ...
    if (iPad) setTimeout(function() { Shining.centerStage('animate') }, 500);
  ...
  Shining.centerStage = function(animate) {
    var top = (window.innerHeight - currentSlide().outerHeight()) / 3;
    if (top < 0) top = 0;
    $('.slide')[!!animate ? 'animate' : 'css']({ top: top }, 100);
  };
  ...
  function currentSlide() {
    return $('#stage .slide:eq(' + Shining.slides._slides.indexOf(Shining.slides.current()) + ') aside');
  }
